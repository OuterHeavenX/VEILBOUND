(() => {
  ...
  function damageEnemy(e){e.lastHitSerial=player.attackSerial;e.hp--;e.flash=.13;e.stun=.16;const dx=e.x-player.x,dy=e.y-player.y,len=Math.hypot(dx,dy)||1;tryMoveEnemy(e,dx/len*18,dy/len*18);spawnParticles(e.x,e.y,'#bdebe1',7,105);screenFlash=Math.max(screenFlash,.045);if(e.hp<=0){saveData.world.defeatedEnemies[e.id]=true;e.dying=deathSeconds(e.type);e.clock=0;spawnParticles(e.x,e.y,'#7fe7e1',14,135);saveGame('ENEMY CLEARED');}}
  ...
  function setEnemyClip(e,chasing){
    const next=e.flash>0?'hurt':e.type==='sentry'?(e.state==='telegraph'?'attack':e.state==='position'?'walk':'idle'):(chasing?'walk':'idle');
    if(next!==e.clip){e.clip=next;e.clock=0;}
  }
  function updateEnemies(dt){for(let i=enemies.length-1;i>=0;i--){const e=enemies[i];e.clock+=dt;if(e.hp<=0){e.dying-=dt;if(e.dying<=0)enemies.splice(i,1);continue;}e.flash=Math.max(0,e.flash-dt);e.stun=Math.max(0,e.stun-dt);e.phase+=dt;if(attackHitsEnemy(e))damageEnemy(e);if(e.stun>0&&e.state!=='disrupted')continue;const dx=player.x-e.x,dy=player.y-e.y,d=Math.hypot(dx,dy)||1;const chasing=e.type==='husk'&&d<HUSK_CHASE_RANGE;if(chasing)tryMoveEnemy(e,dx/d*e.speed*dt,dy/d*e.speed*dt);if(e.type==='sentry')updateSentry(e,dt,d,dx,dy);e.faceX=dx/d;e.faceY=dy/d;setEnemyClip(e,chasing);if(d<player.radius+e.radius+3)hurtPlayer(e.x,e.y);}}
  ...
  function refreshDebugText(){
    ...
    const lines=[
      ...
      `ENEMY ART ${(()=>{const t=['husk','sentry'].filter(k=>Sprites.enemyReady(k,'idle'));return t.length?t.join(' '):'procedural';})()}`,
  ...
  function drawWorld(ctx){const view=getView(),room=rooms[player.room];ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle='#070a0d';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.save();ctx.translate(view.offsetX,view.offsetY);ctx.scale(view.scale,view.scale);ctx.fillStyle=room.ground;ctx.fillRect(0,0,WORLD.width,WORLD.height);drawRoomDetails(ctx,room);ctx.fillStyle='#111817';for(const w of room.walls){if(w.hidden)continue;ctx.fillRect(w.x,w.y,w.w,w.h);ctx.strokeStyle='rgba(190,220,210,.08)';ctx.strokeRect(w.x+.5,w.y+.5,w.w-1,w.h-1);}for(const exit of room.exits){ctx.fillStyle='rgba(127,231,225,.08)';ctx.fillRect(exit.x,exit.y,exit.w,exit.h);}if(room.details==='town')drawTownOver(ctx);drawResonanceNodes(ctx,room);drawProps(ctx,player.room);drawInteractables(ctx,player.room);drawProjectiles(ctx);drawEnemies(ctx);drawParticles(ctx);drawKael(ctx);drawResonancePulse(ctx);if(debugEnabled)drawDebugShapes(ctx,room);ctx.restore();if(screenFlash>0){ctx.fillStyle=`rgba(127,231,225,${Math.min(.34,screenFlash)})`;ctx.fillRect(view.offsetX,view.offsetY,view.drawW,view.drawH);}}
  ...
  // The Sentry's telegraph and disruption rings carry the readability the encounter is
  // tuned around, so they draw over the sprite exactly as they did over the procedural body.
  function drawEnemyOverlay(ctx,e){
    if(e.type!=='sentry'||e.hp<=0)return;
    const cfg=EnemyRegistry.sentry;
    if(e.state==='telegraph'){
      const progress=1-Math.max(0,e.stateTimer)/cfg.attack.telegraphSeconds;
      ctx.strokeStyle=`rgba(255,214,137,${.45+progress*.5})`;ctx.lineWidth=3+progress*3;
      ctx.beginPath();ctx.arc(e.x,e.y,25+progress*9,0,Math.PI*2);ctx.stroke();
      ctx.setLineDash([7,7]);ctx.beginPath();ctx.moveTo(e.x,e.y);ctx.lineTo(e.x+e.aimX*155,e.y+e.aimY*155);ctx.stroke();ctx.setLineDash([]);
    }else if(e.state==='disrupted'){
      ctx.strokeStyle='#7fe7e1';ctx.lineWidth=4;
      ctx.beginPath();ctx.arc(e.x,e.y,29+Math.sin(e.phase*8)*3,0,Math.PI*2);ctx.stroke();
    }
  }
  function drawEnemySprite(ctx,e){
    const clip=e.hp<=0?'death':e.clip;
    if(!Sprites.enemyReady(e.type,clip))return false;
    ctx.fillStyle='rgba(0,0,0,.24)';ctx.beginPath();ctx.ellipse(e.x,e.y+10,17,7,0,0,Math.PI*2);ctx.fill();
    if(e.flash>0)ctx.globalAlpha=.75;
    Sprites.drawEnemy(ctx,e.type,clip,{x:e.x,y:e.y,facingX:e.faceX,facingY:e.faceY,seconds:e.clock});
    ctx.globalAlpha=1;
    return true;
  }
