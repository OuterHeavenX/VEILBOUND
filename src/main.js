(() => {
  ...
  const rooms = {
    ...
    archiveVestibule: {
      ...
      exits:[{x:420,y:0,w:120,h:40,room:'archiveThreshold',spawnX:480,spawnY:465,entry:'south'},{x:420,y:500,w:120,h:40,room:'archiveRotunda',spawnX:480,spawnY:68,entry:'north',requiresFlag:'archive.vestibule.sealOpen'}],
    ...
    archiveRotunda: {
      ...
      walls:[{x:0,y:0,w:420,h:38},{x:540,y:0,w:420,h:38},{x:0,y:502,w:420,h:38},{x:540,y:502,w:420,h:38},{x:0,y:0,w:38,h:540},{x:922,y:0,w:38,h:540},{x:102,y:110,w:104,h:104,hidden:true},{x:754,y:110,w:104,h:104,hidden:true}],
      exits:[{x:420,y:0,w:120,h:40,room:'archiveVestibule',spawnX:480,spawnY:465,entry:'south'}],
  ...
  function closedDoors(roomId){const room=rooms[roomId];return (room&&room.doors||[]).filter(door=>!hasFlag(door.flag));}
  function collidesWithWalls(roomId,x,y,radius){const room=rooms[roomId];if(!room)return false;if(room.walls.some(w=>rectCircleOverlap(w,x,y,radius)))return true;return closedDoors(roomId).some(door=>rectCircleOverlap(door,x,y,radius));}
  ...
  function updateRoomMechanisms(){const room=rooms[player.room];if(!room||!room.switches)return;for(const sw of room.switches){if(hasFlag(sw.flag))continue;if(Math.hypot(player.x-sw.x,player.y-sw.y)<=player.radius+sw.radius){setFlag(sw.flag,true);spawnParticles(sw.x,sw.y,'#7fe7e1',18,110);screenFlash=Math.max(screenFlash,.055);saveGame('MECHANISM ACTIVATED');startDialogue([{speaker:'THE AXIOM',text:'ARCHIVE SEAL RELEASED.'}]);}}}
  function spawnRoomEnemies(){enemies.length=0;projectiles.length=0;for(const def of rooms[player.room].enemies||[]){if(def.persistent&&saveData.world.defeatedEnemies[def.id])continue;const cfg=EnemyRegistry[def.type];enemies.push({...def,radius:cfg.radius,hp:cfg.maxHp,maxHp:cfg.maxHp,speed:cfg.moveSpeed,flash:0,stun:0,lastHitSerial:-1,phase:Math.random()*Math.PI*2,state:'observe',stateTimer:.45+Math.random()*.35,aimX:0,aimY:1,faceX:0,faceY:1,clock:Math.random()*3,clip:'idle',dying:0,progressionRewarded:false});}}
