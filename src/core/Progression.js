(() => {
  ...
  function setMenuOpen(open) {
    if (touchControls) touchControls.hidden = Boolean(open);
  }
  ...
  function init(options) {
    ...
    const Menu = window.Veilbound.CharacterMenu;
    if (Menu) Menu.init({ getSnapshot: snapshot, onOpenChange: setMenuOpen });
  }

  function setActive(active) {
    const Menu = window.Veilbound.CharacterMenu;
    if (Menu) Menu.setActive(active);
  ...
  function updateControls() {
    ...
    if (down && !menuGamepadWasDown) Menu.toggle();
  ...
  window.Veilbound.Progression = {
    ...
    isMenuOpen() {
      const Menu = window.Veilbound.CharacterMenu;
      return Boolean(Menu && Menu.isOpen());
    },
